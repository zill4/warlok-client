...
export class CardSystem {
  ...
  private updateCardScaling() {
    ...
    this.baseCardYPosition =
      containerHeight - this.scaledCardHeight / 2 - marginBottom;
  ...
  public handleMouseMove(clientX: number, clientY: number) {
    ...
    if (
      this.hoveredCard &&
      (!intersects.length || intersects[0].object !== this.hoveredCard)
    ) {
  ...
  private updateHandDisplay() {
    this.cards.forEach((card, index) => {
      ...
      const totalCardsWidth =
        this.cards.length * this.scaledCardWidth +
        (this.cards.length - 1) * this.scaledCardSpacing;
      const startX = this.uiCamera.right / 2 - totalCardsWidth / 2;
      const xPos =
        startX + index * (this.scaledCardWidth + this.scaledCardSpacing);
        ...
        textureLoader.load(
          ...
          (texture) => {
            ...
            const baseResolution = Math.min(
              384,
              Math.max(256, containerWidth * 0.15)
            );
            ...
            ctx.drawImage(
              ...
              0,
              texture.image.width,
              texture.image.height, // Source dimensions
              0,
              0,
              canvas.width,
  ...
  public removeCard(index: number) {
    ...
    if (removedCard) {
      const handIndex = this.localPlayer
        .getHand()
        .findIndex(
          (c) =>
  ...
  private updateSelectedCardsPosition() {
    ...
    const targetY = this.uiCamera.top * 0.8; // Position at about 33% from the top
