...
export class BoardManager {
  ...
  public createBoard() {
    ...
    const offset =
      (BOARD_CONFIG.SIZE * BOARD_CONFIG.SQUARE_SIZE) / 2 -
      BOARD_CONFIG.SQUARE_SIZE / 2;
    ...
    for (let z = 0; z < BOARD_CONFIG.SIZE; z++) {
      for (let x = 0; x < BOARD_CONFIG.SIZE; x++) {
        ...
        const material = new THREE.MeshStandardMaterial({
          color: isWhite
            ? BOARD_CONFIG.COLORS.WHITE
            : BOARD_CONFIG.COLORS.BLACK,
          emissive: 0x000000, // Ensure no emission
        });
    ...
    const sides: FrameSide[] = [
      ...
      {
        pos: [0, 0, -frameSize / 2],
        scale: [frameSize, frameHeight, frameThickness],
      },
      // South
      {
        pos: [0, 0, frameSize / 2],
      ...
      {
        pos: [frameSize / 2, 0, 0],
        scale: [frameThickness, frameHeight, frameSize],
      },
      // West
      {
        pos: [-frameSize / 2, 0, 0],
  ...
  public setupInitialPieces() {
    ...
    const pieceSetup = [
      ...
      {
        type: "pawn",
        positions: Array.from({ length: 8 }, (_, i) => [i, 6]),
        color: "white",
      },
    ];
  ...
  public movePiece(piece: ChessPiece, newX: number, newZ: number) {
    ...
    if (targetPiece && targetPiece.color !== piece.color) {
      console.log(
        `Capturing ${targetPiece.color} ${targetPiece.type} at ${newX},${newZ}`
      );
    ...
    const tween1 = new Tween({ progress: 0 })
      ...
      .onUpdate((obj) => {
        ...
        if (cardMesh && cardStartPosition && cardEndPosition) {
          ...
          const cardX =
            cardStartPosition.x +
            (cardEndPosition.x - cardStartPosition.x) * obj.progress * 0.5;
          const cardZ =
            cardStartPosition.z +
            (cardEndPosition.z - cardStartPosition.z) * obj.progress * 0.5;
    ...
    const tween2 = new Tween({ progress: 0 })
      ...
      .onUpdate((obj) => {
        ...
        if (cardMesh && cardStartPosition && cardEndPosition) {
          ...
          const cardX =
            cardStartPosition.x +
            (cardEndPosition.x - cardStartPosition.x) *
              (0.5 + obj.progress * 0.5);
          const cardZ =
            cardStartPosition.z +
            (cardEndPosition.z - cardStartPosition.z) *
              (0.5 + obj.progress * 0.5);
  ...
  public getBoardSquares(): THREE.Object3D[] {
    return this.board.children.filter(
      (child) =>
    ...
    if (intersects.length > 0) {
      ...
      if (this.selectedPiece && this.possibleMovesHighlighted) {
        ...
        const isValidMove = validMoves.some(
          (move) => move.x === gridPosition.x && move.z === gridPosition.z
        );
  ...
  private clearHighlights() {
    this.highlightedSquares.forEach((square) => {
      if (square instanceof THREE.Mesh) {
        ...
        const isWhite =
          (this.getGridPosition(square.position).x +
            this.getGridPosition(square.position).z) %
            2 ===
          0;
